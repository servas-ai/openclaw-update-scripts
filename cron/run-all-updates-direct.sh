...
if [[ -n "${OPENCLAW_BIN:-}" ]]; then
  if ! openclaw_validate_out="$($OPENCLAW_BIN config validate 2>&1)"; then
...
update_npm_if_needed() {
  ...
  if [[ "$pkg" == "openclaw" ]]; then
    ...
    cmd="\"$OPENCLAW_BIN\" update --channel stable --yes"
    key="openclaw"
...
if [[ "$TELEGRAM_NOTIFY" == "1" ]] && [[ -n "${OPENCLAW_BIN:-}" ]]; then
  ts="$(date '+%d.%m.%Y %H:%M')"
  ...
  if ! env -u OPENCLAW_GATEWAY_URL "$OPENCLAW_BIN" message send \
    ...
    echo "[run-all-updates-direct] WARN: Telegram Abschlussnachricht konnte nicht gesendet werden" >&2
  fi
fi
